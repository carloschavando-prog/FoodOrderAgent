...
def main():
    ...
    matched, unmatched = 0, []
    for raw_mat_num, pr in price_map.items():
        info    = info_map.get(raw_mat_num, {})
        mat_num = str(raw_mat_num).upper()
        ...
        if item_id:
            ...
            normalization = item_map["normalization_by_apn"].get(mat_num)
            if normalization:
                quantity = normalization["unit_quantity"]
                payload.update({
                    **normalization,
                    "unit_price": round(pr["price"] / quantity, 4),
                })
            sb_upsert(
                "pricing",
                payload,
